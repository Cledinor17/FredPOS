...
export default function CategoriesPage() {
  ...
  const categoriesWithCounts = useMemo(() => {
    ...
    return categories.map((category) => {
      ...
      const nameCount = byCategoryName.get(category.name.trim().toLowerCase()) ?? 0;
  ...
  async function toggleActive(item: CatalogCategory) {
    ...
    try {
      const updated = await updateCategory(business, item.id, { isActive: !item.active });
      setCategories((prev) =>
        prev.map((category) => (String(category.id) === id ? updated : category))
      );
    } catch (e) {
      setError(getErrorMessage(e));
    } finally {
  ...
  async function handleRename(item: CatalogCategory) {
    ...
    try {
      const updated = await updateCategory(business, item.id, { name: trimmed });
      setCategories((prev) =>
        prev.map((category) => (String(category.id) === id ? updated : category))
      );
    } catch (e) {
    ...
    <div className="space-y-6">
      <section className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Categories produits</h1>
            <p className="text-slate-500 mt-1">Organise ton catalogue par familles de produits.</p>
          </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <form onSubmit={handleCreateCategory} className="lg:col-span-1 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
          <h2 className="text-base font-bold text-slate-900">Nouvelle categorie</h2>
        ...
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <h2 className="text-base font-bold text-slate-900">Liste des categories</h2>
            <input
              value={query}
              ...
              {filtered.map((item) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">{item.productsCount} produits associes</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={busyId === String(item.id)}
                      title={item.active ? "Desactiver la categorie" : "Activer la categorie"}
                      className={
                        item.active
