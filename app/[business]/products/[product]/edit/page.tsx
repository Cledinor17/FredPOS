...
function validateForm(state: ProductFormState) {
  ...
  if (!isNumberLike(state.stock) || Number(state.stock) < 0) return "Le stock est invalide.";
    ...
    <div className="space-y-6">
      <section className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Modifier produit</h1>
            <p className="text-slate-500 mt-1">Mets a jour les informations puis enregistre.</p>
          </div>
      ...
      <form onSubmit={onSubmit} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-5">
        {loading ? <div className="py-6 text-center text-slate-500">Chargement du produit...</div> : null}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Active">
            <select
              ...
              onChange={(event) => setField("active", event.target.value === "1")}
              className="w-full rounded-xl border border-slate-300 px-4 py-2.5 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
            >
