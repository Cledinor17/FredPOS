...
function resolveProductImageUrl(imagePath: string | null): string | null {
  if (!imagePath) return null;
  if (imagePath.startsWith("http://") || imagePath.startsWith("https://") || imagePath.startsWith("data:")) {
    return imagePath;
  }
  if (!API_ORIGIN) return imagePath;
  if (imagePath.startsWith("/")) return `${API_ORIGIN}${imagePath}`;
...
export default function ProductsPage() {
  ...
  async function onDelete(item: CatalogProduct) {
    if (!business) return;
    if (item.soldCount > 0) {
      setError("Suppression interdite: ce produit a deja ete vendu au moins une fois.");
      return;
    }
    ...
    try {
      ...
      setAllProducts((prev) => prev.filter((product) => String(product.id) !== id));
    } catch (e) {
  ...
  async function onToggleActive(item: CatalogProduct) {
    ...
    try {
      ...
      setAllProducts((prev) => prev.map((product) => (String(product.id) === id ? updated : product)));
    } catch (e) {
      setError(getErrorMessage(e));
    } finally {
      setUpdatingId("");
  ...
  const lowStockCount = allProducts.filter((item) => item.stock > 0 && item.stock <= 10).length;
  ...
  return (
    <div className="space-y-6">
      <section className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1">Gere les references, prix et niveaux de stock.</p>
          </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard label="Total produits" value={String(allProducts.length)} tone="text-indigo-700" />
        <StatCard label="Stock faible" value={String(lowStockCount)} tone="text-amber-700" />
        <StatCard label="Ruptures" value={String(outOfStockCount)} tone="text-rose-700" />
      </section>
      ...
      <section className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        ...
        {loading ? (
          <div className="py-8 text-center text-slate-500">Chargement des produits...</div>
        ) : (
            ...
            <table className="w-full min-w-[760px] text-sm">
              ...
              <tbody>
                {products.map((item) => {
                  ...
                  const imageSrc = brokenImages[itemId] ? null : resolveProductImageUrl(item.imagePath);
                  ...
                  <tr key={itemId} className="border-b last:border-0">
                    ...
                    <td className="py-3 pr-3 text-slate-700">${item.price.toFixed(2)}</td>
                    <td className="py-3 pr-3">
                      <span
                        className={
                          item.stock === 0
                    ...
                    <td className="py-3 pr-3 text-slate-700">{item.soldCount}</td>
                    <td className="py-3 pr-3">
                    ...
                    <td className="py-3">
                      <div className="flex items-center gap-2">
                        <Link
                          href={business ? `/${business}/products/${itemId}/edit` : "/"}
                        ...
                        <button
                          ...
                          disabled={updatingId === itemId || deletingId === itemId}
                          title={item.active ? "Desactiver le produit" : "Activer le produit"}
                          className={
                            item.active
                        ...
                        >
                          {item.active ? <Power className="h-4 w-4" /> : <PowerOff className="h-4 w-4" />}
                        </button>
                        <button
                          onClick={() => onDelete(item)}
                          disabled={deletingId === itemId || updatingId === itemId || item.soldCount > 0}
                          title={
                            item.soldCount > 0
