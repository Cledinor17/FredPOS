...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!data?.user) {
        setErr("Connexion etablie, mais impossible de charger le profil (/api/me). Verifie le backend.");
        return;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50">
      ...
      >
        ...
        <button
          className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl px-4 py-3 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={loading}
        >
          {loading ? (
